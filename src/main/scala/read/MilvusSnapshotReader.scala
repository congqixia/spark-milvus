...
/** One column group within a StorageV2 segment.
  ...
  *   fieldID (>= 100); for multi-field groups it's the smallest unused int < 100.
  *   `-1L` is the sentinel for "unknown slot" (e.g. snapshot-JSON DTO path that
  *   doesn't carry the AVRO slot id) and disables slot-based dedup. We
