 ...
 * 
 * Usage:
 * spark.conf.set("spark.sql.extensions", "com.zilliz.spark.connector.extensions.VectorSearchExtensions")
 * 
 ...
 *   .config("spark.jars", "/path/to/milvus-spark-connector.jar")
 *   .getOrCreate()
 */
...
object CosineSimilarityFunction extends VectorFunctionDescription {
  ...
  override def builder: Seq[Expression] => Expression = {
    case Seq(vector1, vector2) => new CosineSimilarityExpression(vector1, vector2)
    case _ => throw new IllegalArgumentException(s"$name requires exactly 2 arguments")
  }
}
...
object VectorKNNFunction extends VectorFunctionDescription {
  ...
  override def builder: Seq[Expression] => Expression = {
    ...
    case Seq(vectors, queryVector, k) => 
      new VectorKNNExpression(vectors, queryVector, k, org.apache.spark.sql.catalyst.expressions.Literal("cosine"))
    case _ => throw new IllegalArgumentException(s"$name requires 3 or 4 arguments")
  }
}
...
object HammingDistanceFunction extends VectorFunctionDescription {
  ...
  override def builder: Seq[Expression] => Expression = {
    case Seq(vector1, vector2) => new HammingDistanceExpression(vector1, vector2)
    case _ => throw new IllegalArgumentException(s"$name requires exactly 2 arguments")
  }
}
...
object JaccardDistanceFunction extends VectorFunctionDescription {
  ...
  override def builder: Seq[Expression] => Expression = {
    case Seq(vector1, vector2) => new JaccardDistanceExpression(vector1, vector2)
    case _ => throw new IllegalArgumentException(s"$name requires exactly 2 arguments")
  }
}