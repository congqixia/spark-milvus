...
case class SegmentBackfillResult(
    ...
    /** Coalesce-mode only: per new-field count of rows whose written value
      * came from the existing Milvus source (source value was non-null, so
      * `coalesce(src, bf)` kept it). Empty in overwrite mode.
      */
    usedSourceByField: Map[String, Long] = Map.empty,
    /** Coalesce-mode only: per new-field count of rows whose written value
      * came from the backfill data file (source was null and the PK matched a
...
case class BackfillResult(
    ...
    /** Coalesce-mode only: aggregated per new-field count of rows that kept
      * the existing Milvus source value. Empty in overwrite mode.
      */
    totalUsedSourceByField: Map[String, Long] = Map.empty,
    /** Coalesce-mode only: aggregated per new-field count of rows that took
      * the backfill data file value. Empty in overwrite mode.
      */
...
object BackfillResult {
  ...
  def success(
    ...
    def sumByField(pick: SegmentBackfillResult => Map[String, Long])
        : Map[String, Long] =
