...
object MilvusBackfill {
  ...
  /** Workaround until Milvus snapshot exposes `FieldBinlog.child_fields`:
    * when the same Milvus fieldID is claimed by multiple V2 column groups
    * (e.g. an original multi-field group at slot < 100 PLUS a single-field
    * group at slot == fieldID written by a prior addfield+backfill), keep
    * the field only in the group with the largest `slotFieldId`.
    *
